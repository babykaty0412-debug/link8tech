...
1. **顧客點餐** → 加幾樣餐點 → 送出訂單（會自動跳到後台，新單置頂出現）
2. **訂單管理** → 點剛剛那筆 → 改狀態、指派送餐人員 → 列表／統計即時同步
3. **廚房出單** → 看到待處理票卡 → 按「完成出餐」
4. **師傅排班** → 點「⚙️ 管理師傅」新增一位 → 指派班別 → 試試同日排兩班的 ⚠ 警示
5. **殺手鐧**：開發者工具切成離線（Network → Offline）→ 點餐送單 → 顯示「已暫存」→ 恢復連線 → 訂單自動補送進後台，**一筆都不會掉**
